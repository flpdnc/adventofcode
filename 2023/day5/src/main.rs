...
fn process_seeds(line: &str) -> Vec<usize> {
    let mut seeds: Vec<usize> = vec![];
    let seed_ranges: Vec<&str> = line.split(' ').collect();
    let mut i = 1;
    while i < seed_ranges.len() {
        let seed_start: usize = seed_ranges[i].parse().unwrap();
        let length: usize = seed_ranges[i+1].parse().unwrap();
        let seed_end: usize = seed_start + length;
